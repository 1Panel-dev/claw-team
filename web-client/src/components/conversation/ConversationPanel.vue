...
const senderMetaMap = computed<Record<string, SenderMeta>>(() => {
    ...
    if (currentAgentDialogue.value) {
        ...
        if (sourceLabel) {
            assignMeta(sourceLabel, {
                roleName: map[currentAgentDialogue.value.source_agent_ct_id ?? ""]?.roleName ?? map[sourceLabel]?.roleName ?? null,
                ctId: currentAgentDialogue.value.source_agent_ct_id,
        ...
        if (targetLabel) {
            assignMeta(targetLabel, {
                roleName: map[currentAgentDialogue.value.target_agent_ct_id ?? ""]?.roleName ?? map[targetLabel]?.roleName ?? null,
                ctId: currentAgentDialogue.value.target_agent_ct_id,
    ...
    for (const instance of addressBookStore.instances) {
        for (const agent of instance.agents) {
            assignMeta(agent.display_name, { roleName: agent.role_name, ctId: agent.ct_id, instanceName: instance.name });
        }
    }
