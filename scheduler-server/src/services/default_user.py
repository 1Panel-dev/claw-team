...
class DefaultUserIdentity:
    ...
    2. ct_id 是面向后续多用户设计保留的联系人身份。
    3. sender_label 继续保持为 User，尽量避免改动现有展示行为。
    """

    internal_id: str = DEFAULT_USER_INTERNAL_ID
    sender_label: str = DEFAULT_USER_LABEL
    ct_id: str = DEFAULT_USER_CT_ID

    @property
    def label_with_ct_id(self) -> str:
        return f"{self.sender_label} ({self.ct_id})"
...
def display_sender_label(*, sender_type: str, sender_label: str | None) -> str:
    ...
    现阶段只有默认用户需要补 CT ID。
    这样旧消息就算数据库里仍然是 `User`，接口层也能统一显示成 `User (CTU-0001)`。
    """

    if sender_type == "user":
        return get_default_user_identity().label_with_ct_id
